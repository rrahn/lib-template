...
# shipped with this file and also available at: https://github.com/seqan/lib_template/blob/master/LICENSE.md
# -----------------------------------------------------------------------------------------------------
...
find_path (LIB_TEMPLATE_TEST_CMAKE_MODULE_DIR NAMES lib_template_test_component.cmake HINTS "${CMAKE_CURRENT_LIST_DIR}/cmake/")
list(APPEND CMAKE_MODULE_PATH "${LIB_TEMPLATE_TEST_CMAKE_MODULE_DIR}")

set (LIB_TEMPLATE_BENCHMARK_CLONE_DIR "${PROJECT_BINARY_DIR}/vendor/benchmark")
set (LIB_TEMPLATE_TEST_CLONE_DIR "${PROJECT_BINARY_DIR}/vendor/googletest")
...
file(MAKE_DIRECTORY ${LIB_TEMPLATE_BENCHMARK_CLONE_DIR}/include/)
file(MAKE_DIRECTORY ${LIB_TEMPLATE_TEST_CLONE_DIR}/googletest/include/)

# ----------------------------------------------------------------------------
# Interface targets for the different test modules in lib_template.
# ----------------------------------------------------------------------------

# lib_template::test exposes a base set of required flags, includes, definitions and
...
add_library (lib_template::test ALIAS lib_template_test)
...
target_link_libraries (lib_template_test_performance INTERFACE "lib_template::test" "gbenchmark")

if (LIB_TEMPLATE_BENCHMARK_ALIGN_LOOPS)
    target_compile_options (lib_template_test_performance INTERFACE "-falign-loops=32")
endif ()

target_include_directories (lib_template_test_performance INTERFACE "${LIB_TEMPLATE_BENCHMARK_CLONE_DIR}/include/")
add_library (lib_template::test::performance ALIAS lib_template_test_performance)
...
target_link_libraries (lib_template_test_unit INTERFACE "lib_template::test" "gtest_main" "gtest")
target_include_directories (lib_template_test_unit INTERFACE "${LIB_TEMPLATE_TEST_CLONE_DIR}/googletest/include/")
add_library (lib_template::test::unit ALIAS lib_template_test_unit)
...
target_link_libraries (lib_template_test_coverage INTERFACE "lib_template::test::unit" "gcov")
add_library (lib_template::test::coverage ALIAS lib_template_test_coverage)
...
target_link_libraries (lib_template_test_header INTERFACE "lib_template::test::unit")
target_link_libraries (lib_template_test_header INTERFACE "lib_template::test::performance")
...
add_library (lib_template::test::header ALIAS lib_template_test_header)

# ----------------------------------------------------------------------------
# Commonly shared options for external projects.
# ----------------------------------------------------------------------------

set (LIB_TEMPLATE_EXTERNAL_PROJECT_CMAKE_ARGS "")
list (APPEND LIB_TEMPLATE_EXTERNAL_PROJECT_CMAKE_ARGS "-DCMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER}")
list (APPEND LIB_TEMPLATE_EXTERNAL_PROJECT_CMAKE_ARGS "-DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}")
list (APPEND LIB_TEMPLATE_EXTERNAL_PROJECT_CMAKE_ARGS "-DCMAKE_INSTALL_PREFIX=${PROJECT_BINARY_DIR}")
list (APPEND LIB_TEMPLATE_EXTERNAL_PROJECT_CMAKE_ARGS "-DCMAKE_VERBOSE_MAKEFILE=${CMAKE_VERBOSE_MAKEFILE}")
